...
Click **Stop** when you are done, or on Vast.ai run `vastai stop instance <instance_id>`. A stopped pod keeps its disk, so you can resume the same pod later with your depot, clones, and files intact. You still pay for storage while stopped.

To free the disk and stop all charges, terminate the pod (Runpod) or **Destroy** it (Vast.ai, `vastai destroy instance <instance_id>`). This deletes the disk and everything on it, so copy out any results first.
...
On startup the pod clones a fixed set of CliMA repos into `/clima/` in the background: ClimaCoupler.jl, ClimaAtmos.jl, ClimaCore.jl, and ClimaLand.jl. Follow clone progress with `tail -f /tmp/clone.log`. To change the set, edit `REPOS` in `start.sh`.
...
Where you set these depends on the mode. For CI on-demand jobs, the `RunpodJobScheduler` sets them when it deploys the pod. For a manual pod, set them in the Runpod template's **Environment Variables** or in Vast.ai's **Docker options** (`-e KEY=value`). For AWS Batch, set them in the job definition's `containerOverrides`.
